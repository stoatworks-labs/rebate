...
card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/rbtest] [--size WxH] [--jobs N]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The leak controls are conditional.** Leak Edge, Leak Warmth and Leak Spread
do nothing until Leak Amount is up, because at the default of zero there is
no leak to place, colour or spread. Each carries the context it needs in
...
**The scan controls depend on Auto Levels.** Black Point and White Point are
the MANUAL scan's; with Auto Levels on they do nothing, correctly. The
default is a manual scan, so they sweep live on the defaults.

**Edge Text On and Frame Number need a rebate.** They are live on the default
35 mm format; in Full there is no film edge to print on, and in 6x6 there are
no frame numbers (120 film carries them on the backing paper).

**An option's range is its element count, an integer's is its real range.**
`rbtest --list` prints both for exactly this reason, and the ends are what get
swept.
...
LEAK = {"Leak Amount": 0.6}
CONTEXT = {
    # No leak at the default, so nothing to place, colour or spread.
    "Leak Edge": LEAK,
    "Leak Warmth": LEAK,
    "Leak Spread": LEAK,
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "rbtest"))
    ...
    scratch = tempfile.mkdtemp(prefix="rbsweep")
